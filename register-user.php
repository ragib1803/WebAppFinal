...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $userInsertSQL = "INSERT INTO user_table (name, password, phone, address, email, birthdate, blood_group) 
                      VALUES ('$name', '$password', '$phone', '$address', '$email', '$birthdate', '$bloodgroup')";
    ...
    if ($conn->query($donorInsertSQL) === TRUE) {
        $response = array(
            "success" => true,
            "message" => "User and Donor registration successful!"
        );
    } else {
        $response = array(
            "success" => false,
